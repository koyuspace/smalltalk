...
exports.prompt = (title, msg, value = '', options) => {
    ...
    if (type === "shapes") {
        var valueStr = `<style>.js-input-shapes{text-align:center}.js-input-shapes input{display:none}.js-input-shapes input+form{display:inline-block;position:relative;white-space:nowrap;color:rgba(255,255,255,0)}.js-input-shapes label{display:inline;padding:20px 10px;text-align:center;background:#fff;border-radius:10px;box-sizing:border-box;width:48%;font-size:48px;cursor:pointer;font-weight:700;vertical-align:middle}</style><div class="js-input-shapes"><form><input type="radio" id="x" value="x" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'x')" /><input type="radio" id="l" value="l" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'l')" /><input type="radio" id="y" value="y" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'y')" /><input type="radio" id="triangle" value="triangle" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'triangle')" /><input type="radio" id="square" value="square" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'square')" /><input type="radio" id="circle" value="circle" name="js-input" data-name="js-input" onlick="localStorage.setItem('shape-input', 'circle')" /></form><label for="l" style="color: #ff00ff;">L</label><label for="x" style="color: #0000ff;">X</label> <label for="y" style="color: #ffff00;">Y</label> <label for="triangle" style="color: #8000ff;">▲</label><label for="square" style="color: #00ff00;">◼</label><label for="circle" style="color: #ff0000;">●</label></div>`;
    }
...
function showDialog(title, msg, value, buttons, options) {
    ...
    for (const el of find(dialog, ['input'])) {
        ...
        if (el.getAttribute('type') === 'radio')
            localStorage.setItem("shape-input", document.querySelector('input[name="js-input"]:checked').value);
            continue;
...
function closeDialog(el, dialog, ok, cancel) {
    ...
    if (localStorage.getItem("shape-input") === "###shape###") {
